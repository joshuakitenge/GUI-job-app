...
class AP_Testing_Update:
    def __init__ (self,new_window,con):
        ...
        self.drop_ap_testing = OptionMenu(new_window, self.clicked,*self.column_headers[1:])
        self.drop_ap_testing.grid(row=2,column=0,sticky=W,pady=10)
    ...
    def Enter_update(self):
        ...
        query = "UPDATE AP_testing SET {} = '{}' WHERE Job_number='{}'".format(column_head,update,job_number)

        self.cur.execute(query)
        
        self.con.commit()

        self.entered = Label(self.new_window,text="Job application {} succesfully updated {} in Aptitude test table".format(job_number,column_head))
